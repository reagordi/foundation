...
class Event extends BaseObject
{
     ...
     * ignore the rest of the uninvolved event handlers.
     */
     ...
     * @see off()
     */
    public static function on(string $class, string $name, callable $handler, mixed $data = null, bool $append = true): void
    {
        $class = ltrim($class, '\\');
     ...
     * to the named event.
     * @param object|string $class the object or the fully qualified class name specifying the class-level event.
     * @param string $name the event name.
     * @return bool whether there is any handler attached to the event.
     */
    public static function hasHandlers(object|string $class, string $name): bool
    {
        ...
        foreach (self::$_eventWildcards as $nameWildcard => $classHandlers) {
            if (!(new WildcardPattern($nameWildcard))->match($classHandlers)) {
                continue;
            }
            foreach ($classHandlers as $classWildcard => $handlers) {
                ...
                foreach ($classes as $className) {
                    if ((new WildcardPattern($classWildcard))->match($className)) {
                        return true;
                    }
                }
            }
     ...
     */
    public static function trigger(object|string $class, string $name, Event $event = null): void
    {
        ...
        foreach ($classes as $class) {
            ...
            foreach ($wildcardEventHandlers as $classWildcard => $handlers) {
                if ((new WildcardPattern($classWildcard))->match($class)) {
